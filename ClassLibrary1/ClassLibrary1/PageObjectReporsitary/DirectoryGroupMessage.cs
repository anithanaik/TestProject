...
{
    public class DirectoryGroupMessage
    {

        IWebDriver DirectoryGroupMessageDriver;

        public DirectoryGroupMessage(IWebDriver driver)
        {
            DirectoryGroupMessageDriver = driver;
            PageFactory.InitElements(this, new RetryingElementLocator(DirectoryGroupMessageDriver, TimeSpan.FromSeconds(90)));
        }

        #region DirectoryGroupMessageDriver elements        
        ...
        #endregion DirectoryGroupMessageDriver elements
        ...
        public void VerifyDirectoryGroupMessage()
        {
            ...
            Assert.IsTrue(DirectoryTopMenu.Displayed);
            Console.WriteLine("Directory link available on the Top Menu Bar");
            ...
            Console.WriteLine("GroupMeesageButton is displayed   " + GroupMeesageButtonDisplayed);
            Assert.IsTrue(GroupMeesageButtonDisplayed);
            GroupMeesageButton.Click();
            PageServices.WaitForPageToCompletelyLoaded(DirectoryGroupMessageDriver, 60);
            ...
            Console.WriteLine("Selected User list's name is verified with Display Name");
            Actions a = new Actions(DirectoryGroupMessageDriver);  
            DirectoryGroupMessageDriver.SwitchTo().Frame(eventFrameEvent);
            a.Click(inputMessage).SendKeys("QA Test for Group Private Message.").Build().Perform();
            DirectoryGroupMessageDriver.SwitchTo().DefaultContent();                      
            Thread.Sleep(1000);            
            MeesgaeGroupSubmitButton.Click();
            Thread.Sleep(1000);
            Console.WriteLine("Message text Typed and Group Private Message Sent");
            PageServices.ScrollPageUptoTop(DirectoryGroupMessageDriver);
            ...
            Console.WriteLine("Verified Private Message Normal page also has display name at To Field.");
